...
const vcard = 'BEGIN:VCARD\n' //GANTI NO DAN NAMA DIBAWAH INI JADI PUNYA LU
            ...
            + 'FN: OWNER SAD BOY\n' //PENGEMBANG BY :
            + 'ORG: RAMA GANS;\n' //MYBOT TEAM
            + 'TEL;type=CELL;type=VOICE;waid=62895355566000:+62895355566000\n' 
            + 'END:VCARD' //APA
prefix = '?'
blocked = []   
limitawal = '200'
...
/******** OWNER NUMBER**********/
const ownerNumber = ["62895355566000@s.whatsapp.net","6283102650464@s.whatsapp.net"] //ganti nomor lu
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo @${num.split('@')[0]}\Selamat datang di group *${mdata.subject}* yang betah ya di sini jangan lupa intro dan sv nomer bot y kaka ku`
			...
			switch(command) {
				...
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 ⚚★𝐗-𝐓𝐞𝐚𝐦★⚚ 」*\n\nDari Grup : ${groupName}\nPengirim : wa.me/${(sender.split('@')[0])}\nPesan : ${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
